...
`;
  ...
  const generateSinglePage = async (faceId: string, pageNum: number, type: ComicFace['type']) => {
      ...
      if (!url) {
          // If image fails, try one more time with a simpler prompt or just mark as failed
          console.error(`Failed to generate image for ${faceId}`);
      }
      
      updateFaceState(faceId, { imageUrl: url || 'FAILED', isLoading: false });
  };
