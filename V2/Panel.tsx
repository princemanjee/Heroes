...
export const Panel: React.FC<PanelProps> = ({ face, allFaces, onChoice, onOpenBook, onDownload, onReset }) => {
    ...
    return (
        <div className={`panel-container relative group ${isFullBleed ? '!p-0 !bg-[#0a0a0a]' : ''}`}>
            ...
            {face.imageUrl && face.imageUrl !== 'FAILED' && <img src={face.imageUrl} alt="Comic panel" className={`panel-image ${isFullBleed ? '!object-cover' : ''}`} />}
            
            {face.imageUrl === 'FAILED' && (
                <div className="w-full h-full flex flex-col items-center justify-center bg-gray-100 p-8 text-center">
                    <span className="text-6xl mb-4">⚠️</span>
                    <p className="font-comic text-2xl text-red-600 uppercase">Art Block!</p>
                    <p className="text-gray-600 text-sm mt-2">The multiverse artist is temporarily unavailable. Try resetting or checking your API quota.</p>
                </div>
            )}
